...
public class NameSplitter {
	public static void readFileAndWrite(String folder) throws NumberFormatException, IOException {
		List<String> files = NameSplitter.listFile(folder);
		...
		for (String file : files) {
			...
			while (true) {
				...
				if (flag == 0) {
					...
					List<String> splittedKeys = NameSplitter.splitKeys(keys);
					NameSplitter.writeKeys(bw, splittedKeys, counter);
				}
			}
			bw.close();
			fw.close();
	...
	public static List<String> listFile(String folder) {
		...
		for (String file : lsfiles) {
			for (String file2 : lsfiles) {
				if (file2.length() > 16 && file2.substring(14, 15).equals(fileOrder.toString())
						&& file2.endsWith("splitted") == false) {
					++fileOrder;
					files.add(file2);
					break;
				}
